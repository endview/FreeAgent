...
const (
	cmdWindowsTestTempRootEnvV1 = "FREEAGENT_CMD_WINDOWS_TEST_TEMP_ROOT_V1"
...
func runWithPrivateWindowsTestTemp(m *testing.M) int {
	// The production-MCP fixture intentionally starts a copied test binary
	// with an empty environment. It must reach its exact helper Test without
	...
	cmdWindowsTestOuterGoTmpDirV1 = cmdWindowsTestOuterGoTmpDirStateV1{
		owner:   true,
...
func privateWindowsTestTempMCPHelperV1(arguments []string) bool {
	_, _, _, helper := mcpProductionHelperArguments(arguments)
	return helper
}
...
func TestPrivateWindowsTestTempNameV1(t *testing.T) {
	name := privateWindowsTestTempNameV1(cmdWindowsTestTempPrefixV1, 17, 123456789)
	if name != ".c-17-123456789" {
		t.Fatalf("private Windows test temp name = %q", name)
	}
	if !validPrivateWindowsTestTempNameV1(name, cmdWindowsTestTempPrefixV1) {
	...
	for _, invalid := range []string{
		...
		if validPrivateWindowsTestTempNameV1(invalid, cmdWindowsTestTempPrefixV1) {
...
func TestInheritedPrivateWindowsTestTempIsValidatedAndReused(t *testing.T) {
	marker := os.Getenv(cmdWindowsTestTempRootEnvV1)
...
func TestInheritedPrivateWindowsTestTempRejectsPresentEmptyAndMissingValues(t *testing.T) {
	marker, present := os.LookupEnv(cmdWindowsTestTempRootEnvV1)
	if !present || marker == "" {
		t.Fatal("owner marker is unexpectedly missing or empty")
	}
	t.Setenv(cmdWindowsTestTempRootEnvV1, "")
	if _, inherited, err := inheritedPrivateWindowsTestTempForOwnerPIDV1(
		cmdWindowsTestTempRootEnvV1,
		cmdWindowsTestTempPrefixV1,
		os.Getpid(),
	...
	t.Setenv(cmdWindowsTestTempRootEnvV1, marker)
...
func TestPrivateWindowsTestTempMarkerCommitsLast(t *testing.T) {
	...
	want := "GOTMPDIR,TEMP,TMP," + cmdWindowsTestTempRootEnvV1
...
func TestMCPProductionHelperBypassesPollutedWindowsTestTempMarker(t *testing.T) {
	t.Setenv(cmdWindowsTestTempRootEnvV1, "deliberately-invalid")
	arguments := []string{
		os.Args[0],
	...
	if !privateWindowsTestTempMCPHelperV1(arguments) {
		t.Fatal("production MCP helper was not recognized with a polluted temp marker")
	}
}
