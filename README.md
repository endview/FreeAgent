...
[`CORE_RUNTIME_V1`](docs/specs/CORE_RUNTIME_V1.md)、
[`CURRENT_STORE_V1`](docs/specs/CURRENT_STORE_V1.md)、
[`CONTROL_API_V1`](docs/specs/CONTROL_API_V1.md) 和
[`CUTOVER_ACCEPTANCE`](docs/CUTOVER_ACCEPTANCE.md)。
...
旧设计证据已统一降级，索引见[旧架构规格](docs/architecture/specs/README.md)；
它们不构成第二套运行协议。

产品目标与功能架构见[产品需求文档](docs/PRD.md)。
...
Run/Attempt 或重放模型。当前验收状态仍以 [`CUTOVER_ACCEPTANCE`](docs/CUTOVER_ACCEPTANCE.md) 为准。
...
[`docs/S3C_REAL_EXPERIMENT.md`](docs/S3C_REAL_EXPERIMENT.md)。
...
[`MODULE_DEVELOPMENT_V1`](docs/MODULE_DEVELOPMENT_V1.md)。
...
当前开发状态的四分类权威索引见
[`CURRENT_CAPABILITIES`](docs/CURRENT_CAPABILITIES.md)。其中 `accepted` 只表示明确范围内的
...
[历史能力成熟度矩阵](docs/RELEASE_MATURITY.md)；当前切换状态只由三份长期编码规格与
