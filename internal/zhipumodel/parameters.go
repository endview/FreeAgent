...
func validateZhipuParameters(canonical json.RawMessage) (validatedParameters, error) {
	var object map[string]json.RawMessage
	decoder := json.NewDecoder(bytes.NewReader(canonical))
	if err := decoder.Decode(&object); err != nil || object == nil {
		return nil, errorsParameterObject()
	}
...
func errorsParameterObject() error { return fmt.Errorf("parameters must be a JSON object") }

func validateParameterTightening(config, request validatedParameters) error {
...
func positiveIntegerParameter(name string, raw json.RawMessage) (uint64, error) {
	text := string(raw)
	...
	if err != nil || value == 0 || value > MaximumOutputTokensV1 {
		return 0, fmt.Errorf("%s must be a positive integer no greater than %d", name, MaximumOutputTokensV1)
	}
	return value, nil
}

func boundedNumberParameter(name string, raw json.RawMessage, minimum, maximum float64) error {
