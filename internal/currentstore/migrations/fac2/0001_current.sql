...
CREATE TABLE model_dispatch_attempts (
    ...
    usage_ledger_ref TEXT NOT NULL CHECK (
        length(usage_ledger_ref) BETWEEN 1 AND 292
        AND usage_ledger_ref = trim(usage_ledger_ref)
    ),
...
CREATE TABLE model_usage (
    ...
    FOREIGN KEY (
        overview_resource_kind,attempt_id,revision,usage_status,
        ledger_sequence
    ) REFERENCES overview_resource_heads(
...
WHEN NEW.attempt_id<>OLD.attempt_id
  ...
  OR NEW.usage_ledger_ref<>OLD.usage_ledger_ref
...
CREATE TABLE loop_frames (
    ...
    usage_ledger_ref TEXT NOT NULL CHECK (length(trim(usage_ledger_ref)) > 0),
...
WHEN NOT EXISTS (
    ...
    WHERE snapshot.run_id=NEW.run_id
      ...
      AND snapshot.usage_ledger_ref=frame.usage_ledger_ref
    ...
    WHERE snapshot.run_id=NEW.run_id
      ...
      AND (
        (snapshot.transition_kind='CANCELLATION'
          ...
          AND snapshot.usage_ledger_ref=previous.usage_ledger_ref
